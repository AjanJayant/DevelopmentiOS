...
- (IBAction)selectContacts:(id)sender {
    ...
    if(isPresent) {
        ...
        if([potGroup isEqualToString: @"" ]) {
            ...
            // This dict sent to all peeps
            NSMutableDictionary * mess = [[NSMutableDictionary alloc] init];
            NSMutableArray * nameNumber = [[NSMutableArray alloc] init];
            nameNumber[0] = [[Globals sharedInstance] userName];
            nameNumber[1] = [[Globals sharedInstance] userNumber];
            
            [mess setValue: nameNumber forKey: addGroup.text];
            
            // Following code used to send a message to each person who has been added notifying them of such
            
            PNChannel *channels[[arr count]];
            for(int j = 0; j < [arr count]; j++) {
                PNChannel *c = [PNChannel channelWithName: [[[Globals sharedInstance] nameNumber] objectForKey:arr[j]] shouldObservePresence:YES];
                channels[j] = c;
                [PubNub sendMessage: mess toChannel:channels[j]];
            }
